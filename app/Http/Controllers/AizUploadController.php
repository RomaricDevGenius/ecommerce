...
class AizUploadController extends Controller
{
    ...
    public function destroy($id)
    {
        ...
        try {
            if (env('FILESYSTEM_DRIVER') != 'local') {
                // Supprimer du stockage cloud
                try {
                    Storage::disk(env('FILESYSTEM_DRIVER'))->delete($upload->file_name);
                    $fileDeleted = true;
                } catch (\Exception $e) {
                    // Ignorer si le fichier n'existe pas dans le cloud
                }
                
                // Supprimer aussi le fichier local s'il existe
        ...
        // Toujours supprimer l'entrée de la base de données
        $upload->delete();
        
        if ($fileDeleted) {
            flash(translate('File deleted successfully'))->success();
        } else {
            flash(translate('File entry deleted from database (file was not found)'))->warning();
        }
